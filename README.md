...
`Eudiplo.Client.Sample.AccessControl` isn't a single console project — it has its own
`Backend/` (the only piece using `Eudiplo.Client`) and `Frontend/` (Lit + TypeScript UI)
subfolders, since the point of that sample is showing the boundary between "your UI" and
"your backend" from the diagram above.
